...
/*
 ...
 Modified by Hassan Albalawi to be used on OpenBCI 32-bit board for AlphaWave project
...
void OpenBCI_32_BLE::initialize(){
...
void OpenBCI_32_BLE::startStreaming(){
  if(useAccel){enable_accel(RATE_50HZ);}
  startADS();
} 

void OpenBCI_32_BLE::sendChannelData(byte sampleNumber){
...
void OpenBCI_32_BLE::stopStreaming(){
  stopADS();
  if(useAccel){disable_accel();}
}

//SPI communication method
byte OpenBCI_32_BLE::xfer(byte _data) 
{
    byte inByte;
...
void OpenBCI_32_BLE::csLow(int SS)
{
  switch(SS){
    case ADS_SS:
...
void OpenBCI_32_BLE::csHigh(int SS)
{
  switch(SS){
    case ADS_SS:
...
void OpenBCI_32_BLE::writeAuxData(){
  for(int i=0; i<3; i++){
...
// void OpenBCI_32_BLE::initialize(int _DRDY, int _RST, int _CS){
void OpenBCI_32_BLE::initialize_ads(){
...
void OpenBCI_32_BLE::updateChannelData(){
    byte inByte;
    int byteCounter = 0;
...
void OpenBCI_32_BLE::setChannelsToEMG(void){
...
void OpenBCI_32_BLE::setChannelsToECG(void){
  channelSettings[0][POWER_DOWN] = NO;        // NO = on, YES = off
  channelSettings[0][GAIN_SET] = ADS_GAIN12;     // Gain setting
  channelSettings[0][INPUT_TYPE_SET] = ADSINPUT_NORMAL;// input muxer setting
  channelSettings[0][BIAS_SET] = NO;    // add this channel to bias generation
...
void OpenBCI_32_BLE::writeChannelSettings(void){
  byte setting;
...
void OpenBCI_32_BLE::writeChannelSettings(char N){
  byte setting;
  if ((N < 1) || (N > 8)) {
	  // Serial0.println("channel number out of range");
	  return;
	  }  // must be a legit channel number
...
// 
void OpenBCI_32_BLE::deactivateChannel(int N)
{
...
void OpenBCI_32_BLE::activateChannel(int N) 
{
	byte setting;  // ,reg  // trying not to use reg
   //check the inputs
  if ((N < 1) || (N > 8)) return;
...
void OpenBCI_32_BLE::changeChannelLeadOffDetect(void)
{
...
void OpenBCI_32_BLE::configureLeadOffDetection(byte amplitudeCode, byte freqCode)
{
...
void OpenBCI_32_BLE::startADS(void)
{
...
boolean OpenBCI_32_BLE::isDataAvailable(void)
{
  return (!(digitalRead(ADS_DRDY)));
}
  
// Stop the continuous data acquisition
void OpenBCI_32_BLE::stopADS()
{
...
void OpenBCI_32_BLE::ADS_writeChannelData(void)
{
...
void OpenBCI_32_BLE::ADS_printDeviceID(void)
{ 
    boolean wasRunning;
...
void OpenBCI_32_BLE::STANDBY() {    // only allowed to send WAKEUP after sending STANDBY
    csLow(ADS_SS);
    xfer(_STANDBY);
    csHigh(ADS_SS);
}

void OpenBCI_32_BLE::RESET() {      // reset all the registers to default settings
    csLow(ADS_SS);
    xfer(_RESET);
...
void OpenBCI_32_BLE::START() {      //start data conversion 
    csLow(ADS_SS);
    xfer(_START);           // KEEP ON-BOARD AND ON-DAISY IN SYNC
    csHigh(ADS_SS);
}

void OpenBCI_32_BLE::STOP() {     //stop data conversion
    csLow(ADS_SS);
    xfer(_STOP);            // KEEP ON-BOARD AND ON-DAISY IN SYNC
    csHigh(ADS_SS);
}

void OpenBCI_32_BLE::RDATAC() {
    csLow(ADS_SS);
    xfer(_RDATAC);      // read data continuous
    csHigh(ADS_SS);
  delayMicroseconds(3);   
}
void OpenBCI_32_BLE::SDATAC() {
    csLow(ADS_SS);
    xfer(_SDATAC);
    csHigh(ADS_SS);
...
byte OpenBCI_32_BLE::ADS_getDeviceID() {      // simple hello world com check
  byte data = RREG(0x00);
...
void OpenBCI_32_BLE::RDATA() {          //  use in Stop Read Continuous mode when DRDY goes low
...
byte OpenBCI_32_BLE::RREG(byte _address) {    //  reads ONE register at _address
...
void OpenBCI_32_BLE::RREGS(byte _address, byte _numRegistersMinusOne) {
...
void OpenBCI_32_BLE::WREG(byte _address, byte _value) { //  Write ONE register at _address
...
void OpenBCI_32_BLE::WREGS(byte _address, byte _numRegistersMinusOne) {
    byte opcode1 = _address + 0x40;   //  WREG expects 010rrrrr where rrrrr = _address
    csLow(ADS_SS);        //  open SPI
    xfer(opcode1);          //  Send WREG command & address
...
//hasan
void OpenBCI_32_BLE::update24bitData(){
  int indexCounter = 0;
  for(int i=0; i<8; i++){
	for(int j=2; j>=0; j--){
	  channelDataRaw[indexCounter] = byte(channelDataInt[i] >> (8*j));
	  indexCounter++;
	}
  }
}
...
void OpenBCI_32_BLE::initialize_accel(byte g){ 
...
void OpenBCI_32_BLE::enable_accel(byte Hz){    // ADD ABILITY TO SET FREQUENCY & RESOLUTION
  for(int i=0; i<3; i++){
...
void OpenBCI_32_BLE::LIS3DH_writeAxisData(void){
  for(int i=0; i<3; i++){
...
byte OpenBCI_32_BLE::LIS3DH_read(byte reg){
...
void OpenBCI_32_BLE::LIS3DH_write(byte reg, byte value){
  csLow(LIS3DH_SS);                  // take spi
...
int OpenBCI_32_BLE::LIS3DH_read16(byte reg){    // use for reading axis data. 
  int inData;  
...
int OpenBCI_32_BLE::getX(){
  return LIS3DH_read16(OUT_X_L);
}

int OpenBCI_32_BLE::getY(){
  return LIS3DH_read16(OUT_Y_L);
}

int OpenBCI_32_BLE::getZ(){
  return LIS3DH_read16(OUT_Z_L);
}

void OpenBCI_32_BLE::LIS3DH_updateAxisData(){
  axisData[0] = getX();
  axisData[1] = getY();
  axisData[2] = getZ();
}
...
void OpenBCI_32_BLE::printRegisterName(byte _address) {
    switch(_address){ 
      case ID: 
        //Serial0.print("ID, "); 
		break;
      case CONFIG1:
        //Serial0.print("CONFIG1, "); 
		break;
      case CONFIG2:
        // Serial0.print("CONFIG2, "); 
		break;
      case CONFIG3:
        // Serial0.print("CONFIG3, "); 
		break;
      case LOFF:
        // Serial0.print("LOFF, "); 
		break;
      case CH1SET:
        // Serial0.print("CH1SET, "); 
		break;
      case CH2SET:
        // Serial0.print("CH2SET, "); 
		break;
      case CH3SET:
        // Serial0.print("CH3SET, "); 
		break;
      case CH4SET:
        // Serial0.print("CH4SET, "); 
		break;
      case CH5SET:
        // Serial0.print("CH5SET, "); 
		break;
      case CH6SET:
        // Serial0.print("CH6SET, "); 
		break;
      case CH7SET:
        // Serial0.print("CH7SET, "); 
		break;
      case CH8SET:
        // Serial0.print("CH8SET, "); 
		break;
      case BIAS_SENSP:
        // Serial0.print("BIAS_SENSP, "); 
		break;
      case BIAS_SENSN:
        // Serial0.print("BIAS_SENSN, "); 
		break;
      case LOFF_SENSP:
        // Serial0.print("LOFF_SENSP, "); 
		break;
      case LOFF_SENSN:
        // Serial0.print("LOFF_SENSN, "); 
		break;
      case LOFF_FLIP:
        // Serial0.print("LOFF_FLIP, "); 
		break;
      case LOFF_STATP:
        // Serial0.print("LOFF_STATP, "); 
		break;
      case LOFF_STATN:
        // Serial0.print("LOFF_STATN, "); 
		break;
      case GPIO:
        // Serial0.print("GPIO, "); 
		break;
...
void OpenBCI_32_BLE::printHex(byte _data){
  // Serial0.print("0x");
